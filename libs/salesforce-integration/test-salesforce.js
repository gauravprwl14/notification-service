...
async function main() {
    ...
    try {
        ...
        console.log(`SALESFORCE_PRIVATE_KEY_PATH: ${process.env.SALESFORCE_PRIVATE_KEY_PATH ? '✓ Set' : '✗ Not set'}`);
        console.log(`SALESFORCE_LOGIN_URL: ${process.env.SALESFORCE_LOGIN_URL ? '✓ Set' : '✗ Not set'}`);
        console.log(`SALESFORCE_API_VERSION: ${process.env.SALESFORCE_API_VERSION ? '✓ Set' : '✗ Not set'}`);
        console.log(`SALESFORCE_EVENT_OBJECT: ${process.env.SALESFORCE_EVENT_OBJECT ? '✓ Set' : '✗ Not set'}\n`);
        ...
        const eventObject = process.env.SALESFORCE_EVENT_OBJECT || 'tyfonenao__Notification__e';
        ...
        const result = await sendNotificationEvent(testEvent, accessToken.access_token, accessToken.instance_url);
    ...
    async function sendNotificationEvent(event, accessToken, instanceUrl) {
        ...
        const apiVersion = process.env.SALESFORCE_API_VERSION || 'v63.0';
        const eventObjectName = process.env.SALESFORCE_EVENT_OBJECT || 'tyfonenao__Notification__e';
